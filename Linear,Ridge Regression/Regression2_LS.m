clc
clear
%Regression - "Least Sqaure" for 2-D input data
...
w = inv(X'*X)*X'*T;         % calculating weight parameters
E = (T - X*w)'*(T - X*w);   % calculating training error
...
E1 = (T1 - Y)'*(T1 - Y);    % calculating training error
Errors(K,2) = E1;           % storing training error
